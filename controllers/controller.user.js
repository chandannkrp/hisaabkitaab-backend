...
export const verifyOTP = async (req, res) => {
  try {
    ...
    if (!user) {
      logger.warn(
        `Invalid or expired OTP entered by : ${email}, IP: ${req.ip}`
      );
...
export const loginUser = async (req, res) => {
  try {
    ...
    if (!user) {
      logger.warn(
        `Login attempt with non-existent email: ${email}, IP: ${req.ip}`
      );
      return res.status(400).json({ message: "User does not exist" });
    }
    ...
    if (!isMatch) {
      logger.warn(
        `Login attempt with incorrect password: ${email}, IP: ${req.ip}`
      );
...
export const authorizeEmail = async (req, res) => {
  try {
    ...
    await sendEmail(
      email,
      "Authorization OTP for transaction",
      "emailOtp.html",
      { name: user.name, otp }
    );
...
export const authorizeEmailOTP = async (req, res) => {
  try {
    const { email, otp } = req.body;
