...
export const getUserClients = asyncHandler(async (req, res) => {
...
export const addNewUserClient = asyncHandler(async (req, res) => {
  const userId = req.user._id;
  ...
  if (req.user.email === email) {
    return res
      .status(400)
      .json({ message: "You cannot add yourself as a client" });
  }
  ...
  if (existingUser) {
    ...
    if (existingActiveRelation) {
      return res
        .status(400)
        .json({ message: "Relation already exists with this client" });
    }
...
export const removeUserClientById = asyncHandler(async (req, res) => {
  const userId = req.user._id;
